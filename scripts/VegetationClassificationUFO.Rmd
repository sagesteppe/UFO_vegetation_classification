...
veg_rf_model <- readRDS(veg_rf_model, 
                        file = paste0('../results/stats/RandomForest', Sys.Date(), '.rds'))
...
cm1 <- confusionMatrix(test_prediction, test$class)
...
While the accuracy for the model was `r toString(round(cm1[["overall"]][["Accuracy"]],3))` (range `r toString(round(cm1[["overall"]][["AccuracyLower"]],2))` - `r toString(round(cm1[["overall"]][["AccuracyUpper"]], 2))`), due to an imbalance in the number of observations per group a more appropriate for evaluating the overall performance of the model, is  Kappa metric which was `r toString(round(cm1[["overall"]][["Kappa"]], 3))`.

Notably, the model has high rates of Specificity, showing that when it predicts a vegetation onto a pixel cell, the prediction is usually correct; less so for prediction of Pinon-Juniper. However, the model suffers from low Sensitivity, indicating that it is unable to detect all occurrences of a vegetation type. For example, the model is only able to appropriately classify half of the occurrences of Sage Steppe and Mixed Mountain Shrub. 
...
```{r make waffle charts of changes in vegetation, echo = F,  fig.height = 8, fig.width = 3, fig.cap = "Changes in Estimated Land Cover between the initial stratification and post-stratification"}
library(waffle)
library(tidyverse)
library(cowplot)
