...
On the whole the new model outperforms the older model in all comparisons except for that the sensitivity of the original PJ classification is higher than that of the secondary PJ classification (`r toString(round(cmOrig[[4]][grep('PJ', rownames(cmOrig[['byClass']])) , grep('Sens', colnames(cmOrig[[4]])) ], 3))` to `r toString(round(cmRestrat[[4]][grep('PJ', rownames(cmRestrat[['byClass']])) , grep('Sens', colnames(cmRestrat[[4]])) ], 3))`). In general their are multiple trade offs in the comparison of models, however the new model is both likely to correctly identify a the strata of a location, and to identify it correctly. 
