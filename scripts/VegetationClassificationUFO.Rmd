...
naip_IR_r <- raster::raster(naip_IR)
naip_NC_r <- raster::raster(naip_NC)
...
glcm.colour <- glcm::glcm(naip_NC_r,
                 window = c(7, 7), shift = list(c(0,1), c(1,1), c(1,0), c(1,-1)), 
                 statistics = c("mean", "variance", 'homogeneity', 'contrast', 'dissimilarity'), 
                 min_x = 1, max_x = 255, na_opt = 'ignore', na_val = 0)
Sys.time()
...
all_textures <- stack(glcm.red, glcm.colour)
...
all_textures <- rast(file.path(path, 'all-textures.tif'))

all_textures <- all_textures[[1:4]]
### USE THIS !!!!!!! NOT WHAT IS BELOW. 

sr <- values(spatSample(all_textures, 100000, as.raster=TRUE))
pca <- prcomp(sr)
plot(pca)
text_PC <- predict(all_textures, pca)
plot(text_PC)
...
texture_PrincComponents <- c(text_PC[[c(1:2)]])
names(texture_PrincComponents) <- paste0('principal_Component_1', 
...
# rm(texture_PrincComponents, all_textures, glcm.red, glcm.colour)
```

NAIP data were processed to create an NDVI band. NDVI is well suited for identifying sparsely vegetated areas. 

```{r create NDVI band, echo = F}

## create an NDVI band from the existing datasets - this helps identify sparsely vegetated areas. 

# (NIR - Red) / (NIR + Red)

naip_ndvi <- (naip_multispectral_br[[4]] - naip_multispectral_br[[1]]) / 
  (naip_multispectral_br[[4]] + naip_multispectral_br[[1]])
...
```{r Export reclassified AIM Points, echo = F, eval = F}
...
```{r buffer all samples to AIM plot sizes, echo = F, eval = F}
...
rm(reg_pts, computer_pts, reg_pts)
```
...
naip_NC <- rast(file.path(
  path, files[grep('NC_naip.tif', files)]))

texture <- rast(file.path(
  path, files[grep('all-textures.tif', files)]))[[c(1:2)]]
texture <- resample(texture, naip_NC, method = 'cubicspline')

PhysioPath <- '/home/reed/AIM_Field/data/processed'
...
predictors <- c(naip_NC, texture, DEM, slope)
names(predictors) <- c('NAIP_red', 'NAIP_green', 'NAIP_blue', 'glcm_mean',
                       'glcm_variance', 'DEM', 'slope')
plot(predictors)

rm(naip_NC, texture, DEM, slope, PhysioPath, UFO_land, files)
```

```{r Extract Predictor Variables to Points}
library(caret)

VegClass <- extract(predictors, rand_pts, method = 'bilinear', weights = T,  fun = mean)
VegClass <- cbind(as.data.frame(rand_pts), VegClass[,2:ncol(VegClass)])
...
seeds[n_folds + 1] <- sample.int(1000, 1) # seed for the final mod

# define the controls for the model generation
...
model_rf <- caret::train(class ~ ., method = "rf", data = train,
...
predict_rf <- terra::predict(object = predictors, model = model_rf, cpkgs = 'caret',
