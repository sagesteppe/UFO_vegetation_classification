...
reclassified <- rast(file.path(here(), 'data/processed/PredictedVegClass-sm.tif'))
...
original_strat <- project(original_strat, crs(reclassified))
orig_strat_resamp <- resample(original_strat, reclassified, method = 'near')
orig_strat_resamp <- mask(orig_strat_resamp, reclassified)

rm(original_strat)

vorig <- as.matrix(orig_strat_resamp, wide = T, na.rm = T)
vclassified <- as.matrix(reclassified, wide = T, na.rm = T)
rm(vorig, vclassified)

opoly <- as.polygons(orig_strat_resamp)
opoly <- sf::st_as_sf(opoly)
