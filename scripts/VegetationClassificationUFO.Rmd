...
strata_waffle <- ggplot(reprocessedGAP, aes(fill = Code, values = Percent)) + 
  ...
  coord_equal() + 
  theme_void() + 
  labs(title = 'land cover in the initial design')  +
...
reclassified <- rast(file.path('/media/sagesteppe/ExternalHD/plot_post_stratification', 'data/processed/PredictedVegClass-sm.tif'))
...
reclass_waffle <- ggplot(vegclass, aes(fill = Stratum, values = Percent)) + 
  ...
  coord_equal() + 
  theme_void() + 
  labs(title = 'land cover based on modeling')  +
  theme(plot.title = element_text(hjust = 0.5), 
       legend.key.size = unit(0.2, "cm"))

plot_grid(strata_waffle, reclass_waffle, ncol = 1)

# show the flow from stratified land cover to reclassified land cover
```
...
```{r reimport data for showing flow from veg classes, echo = F}
...
ab <- left_join(rvals, ltable, by = c('Reclassified' = 'num')) %>% 
  left_join(., reprocessedGAP %>% 
              select(RasterValue, Code),
            by = c('Original' = 'RasterValue')) %>% 
  left_join(., area %>% select(num, Cells),
            by = c('Original' = 'num')) %>% 
  rowwise() %>% 
  mutate(value = round( n/Cells, 5)) %>% 
  select(IDSource = Original, 
         Source = Code, Target = Stratum, value)

rvals <- pivot_wider(rvals, names_from = 'Reclassified', values_from = 'n') %>% 
...
chordDiagram(rvals, transparency = 0.5, directional = 1, grid.col = all_pals)
