...
PhysioPath <- '/media/reed/ExternalHD/AIM_Field_rasters'
...
VegClass <- extract(predictors, veg_pts, method = 'simple',  fun = mean, weights = T)
...
veg_rf_model <- randomForest(class~., data=train, proximity=TRUE, 
                             ntree = nTT, mtry = 4) # we can only run so many trees
