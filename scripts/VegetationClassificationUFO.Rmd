...
```{r buffer points from GE and AIM , echo = F, eval = F}
...
testOriginal <- veg_pts[-split_obs,]

rm(VegClass, split_obs)
```

The dataset of `r toString()` classified points were partioned into a `r toString(1-train_prop)` test and training set `r toString(train_prop)` using caret, the dataset was not balanced, see table XX for sample sizes.  
